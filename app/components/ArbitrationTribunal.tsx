...
export function ArbitrationTribunal({ isDark = true }: { isDark?: boolean }) {
  ...
  const runAIArbitration = async () => {
    ...
    // First submit primary claims as evidence if they exist
    if (claimantClaim) {
      await submitEvidenceToBackend('claimant', claimantClaim);
    }
    if (respondentDefense) {
      await submitEvidenceToBackend('respondent', respondentDefense);
    }

    // Then submit any additional evidence items
